...
class PacketType:
    ...
    # Static functions for verifying the size of a packet is valid for the
    # packet type.
    @staticmethod
...
class Packet:
    ...
    # Use the PacketType enum to verify the reported type and size of the
    # packet.
    @staticmethod
    ...
    def ParseData(self, data):
        ...
        if self.type == PacketType.START:
            # Extract stage.
            self.stage = struct.unpack(Packet.format_strings["start"], data[
                                       0:PlayerParams.HEADER_SIZE])[0]

            # Extract player params.
            num_param_bytes = len(data) - PlayerParams.HEADER_SIZE
            if not num_param_bytes % PlayerParams.SIZE == 0:
                print("Invalid number of bytes for player params",
                      num_param_bytes, PlayerParams.SIZE)
                return False
            self.player_params = []
            for i in range(0, int(num_param_bytes / PlayerParams.SIZE)):
        ...
        # 57 bytes repeated player update
            ...
            (self.frame_count, self.rng_seed) = \
                struct.unpack(Packet.format_strings["update"], data[
                              0:PlayerUpdate.HEADER_SIZE])

            # Extract player updates.
            num_update_bytes = len(data) - PlayerUpdate.HEADER_SIZE
            if not num_update_bytes % PlayerUpdate.SIZE == 0:
                print("Invalid number of bytes for player updates",
                      num_update_bytes, PlayerUpdate.SIZE)
                return False
            self.player_updates = []
            for i in range(0, int(num_update_bytes / PlayerUpdate.SIZE)):
        ...
        # 1 byte win condition
            ...
            self.win_condition = struct.unpack(
                Packet.format_strings["end"], data)[0]
